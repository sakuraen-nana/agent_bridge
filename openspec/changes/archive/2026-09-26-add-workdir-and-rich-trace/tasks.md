...
- [x] 6.2 Windows 被控端实跑：同上，另验证无效路径拒绝启动与留痕分节排版的实际呈现；验证：命令与输出留证 —— 证据（2026-09-26 实跑，环境按既有约定以通用角色指代）：被控机以 `--workdir <一个项目目录>` 启动后，`hello` 的 `cwd` 字段、`exec`（不带 `--cwd`）与 `exec --cwd .` 三者均返回该目录；`exec --cwd src` 返回 400「cwd 不存在或不是目录」——该目录下无 `src` 而工具目录有，**反证相对 cwd 基准已切换**；`download` 相对路径同理（取回工作目录内的一个文件 1072 字节，客户端哈希 == 本机计算 == 被控机 `certutil` 三方一致；改取工具目录下的文件名则 404）；无效目录拒绝启动与留痕排版见 §6.4 / §6.5

### 待用户验收清单（需在被控机上人工操作）

- [x] 6.3 在被控机上取到本次提交（`git pull --ff-only`）后重启服务：`python run.py server --workdir <项目目录>`；**预期**：横幅「工作目录」显示该目录并标注其来自启动参数，服务正常提供全部能力。**注意**：token 随重启轮换，需把控制台新 token 抄入客户端侧 token 文档后通道才恢复 —— 已达：横幅为「工作目录 : <该目录>（来自 --workdir）」，新 token 抄入客户端侧 token 文档后通道恢复
- [x] 6.4 在被控机以 `--workdir` 指向一个不可访问的目录启动（Windows 上可构造为无读取权限的目录）；**预期**：输出指明原因的错误并以非零码退出，端口不被监听 —— 已达（不存在的路径一例）：另开窗口以 `--workdir` 指向不存在的路径启动，输出「目录无效：路径不存在」并退出，未启动服务、未占端口；**「不可访问」一例在 Windows 上未构造**（该子情形已在 Linux 侧以非特权用户端到端验证，见 §1.2）
- [x] 6.5 从客户端侧触发一次 exec 与一次 download，观察被控机控制台；**预期**：控制台按小节排版，参数与结果完整可读，download 同时显示原始 `path` 与绝对路径 —— 已达：使用方在被控机控制台确认，exec 与 download 两条留痕均呈现分节标题（`▸ 请求参数` 等）、字段按等号对齐、参数完整
- [x] 6.6 在被控机另跑一次不带 `--workdir` 的启动（在项目目录下 `cd` 后启动）；**预期**：横幅标注「未指定 --workdir，取启动时目录」，行为与旧版一致 —— 已达：此前一次实例即为不带参数启动，横幅标注「未指定 --workdir，取启动时目录」，工作目录为启动时目录；`exec` 缺省 cwd 与相对 `cwd` 均以它为基准，与旧版一致

## 7. 文档与收尾

- [x] 7.1 `README.md`：补启动参数说明（`--workdir` 用法、必须在 `server` 之后、`run.py --workdir X` 不成立）与留痕形态说明；验证：照 README 步骤可复现，且守卫 grep（机器名 / IP / 部署路径）零命中 —— 证据：提交 `f2b67ca`；守卫 grep 在 `AGENTS.md` / `README.md` 上零命中（除 LICENSE 作者署名这一既有豁免项）。**更正**：本条原写作「全仓零命中」，实为误述——当时全仓范围内仍存本机绝对路径（已随后修正）与守卫模式串的自匹配，守卫的可执行化缺口见 add-agent-bridge §8.6
- [x] 7.2 分提交推送（实现、测试、文档各自成提交）；验证：`git log --oneline` 与远端一致、`git status` 干净 —— 证据：提交 `f40907d`（实现）/ `1547022`（测试）/ `f2b67ca`（文档）；**已推送**：`0e2b3d5..f2b67ca  main -> main`，`git status -sb` 显示与 `origin/main` 同步；归档序列另含版本推进提交 `4a5538c`（`0.1.0` → `0.2.0`，按 AGENTS.md 开发流程第 6 条）与归档提交 `b06fa61`
- [x] 7.3 证据登记：勾选各项并附证据（提交哈希 / 命令 / 退出码 / 产物名）；未在目标平台实跑的不勾选 —— 证据：本次更新即登记，随后被控机恢复、§6.2 与 §6.3–6.6 的实跑证据补齐并勾选（见各项）；§8 三项跟进项按范围约定保持未勾选
