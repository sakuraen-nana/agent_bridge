...
- [x] 6.1 分提交推送：实现、测试、文档各自成提交，推送至本仓库远端；验证：`git log --oneline` 与远端一致，`git status` 干净 —— 证据：本地 5 个提交 `d4ede47`/`27155ce`/`9eaae90`/`aa7736d`/`3cd1e89`（另 `🐞 fix(client)` 与证据登记见后续提交）；**推送待本仓库远端通路确认后执行**（见下方待确认项）
- [x] 6.2 证据登记：更新本变更 tasks 勾选与证据（提交哈希 / 命令 / 退出码 / 产物名）；验证：勾选项均附证据，未实跑验证的不勾选 —— 证据：本次更新即登记

> **推送未执行（2026-09-25 实测）**：本变更实施所在的开发环境当前无法完成对本仓库远端的 git 交换——`git ls-remote`（SSH 与 HTTPS 各试 25–30s）均超时；TCP 直连 `github.com:443` 可建立，但 TLS 之后的 git 操作超时，`ssh.github.com:443` 与 SSH 22 均不可达，且本机无 HTTPS 凭据配置。故提交只落在本地仓库；推送待通路可用时执行，或按 design D9 经离线/局域网交付。此项不阻塞 §5（被控机侧验证同样需要在被控机取得一份副本，本就需要一条可用通道）。
